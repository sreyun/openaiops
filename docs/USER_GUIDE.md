...
- **复制**：拖选后按 **Ctrl+C**，或直接**在高亮上点右键**即复制（点在别处不拦，浏览器自己的菜单照常出来，也不会冲掉你正准备粘贴的剪贴板）；标题栏还有一个**复制按钮**——有选区复制选区，没有就复制整屏。没有选中内容时 Ctrl+C 仍然是 SIGINT，不会抢走中断。终端不再有自绘的右键菜单。
...
**远程桌面（Web 推流）**：浏览器直连远程桌面，Windows 锁屏 / 登录界面 / Ctrl+Alt+Del 都能用（需以服务方式安装 Agent）。

- **只传变化的画面**：屏幕按 128×128 切块逐块比对，只把变化的块编码发出去。在浏览器地址栏敲一个字，
  过去要重编一整屏（1080p 下 200–400KB），现在通常只有几 KB——同一条链路上，"点一下等三五秒"变成
  一个网络往返。整屏大面积变化（切窗口、放视频）时自动退回整帧，那时整帧压得更好；另外每 5 秒补一张
  整帧作为关键帧，用于录像回放与丢包自愈。
- **只保最新画面**：浏览器跟不上时，服务端丢掉积压的旧帧而不是排队——队列有多深，画面就落后多少。
  上行的鼠标移动同理，只保最新位置（按下 / 抬起 / 滚轮 / 按键一个都不丢）。
- **键盘按真键注入**：字符先按远端当前键盘布局还原成"哪个键 + 要不要按 Shift"，再以扫描码注入。
  这样**命令行窗口（cmd / PowerShell）、游戏、全屏对话框**都能收到——只发 Unicode 字符事件的做法
  在这些地方是无效的（这正是过去"远程桌面里打开 cmd 敲不进字"的原因）。中日韩等布局打不出的字符
  仍走 Unicode 通道。
- **会话切换不掉线**：Windows 登录 / 注销 / 切换用户会让 Agent 在新会话里重开桌面进程，此时画面会短暂
  显示"正在恢复"，随后自动接上——不需要关掉窗口重进。
- **分辨率**：画面编码分辨率始终跟着你的窗口走（窗口够大就是 1:1 原生，不再固定降采样）。
  但缩放变不出不存在的像素——远端只有 1024×768 时，放大到 2560 的窗口必然糊。所以工具栏还有一个
  **分辨率**下拉：选「匹配我的窗口」会把**远端的显示分辨率**切到最贴近你窗口比例的模式（mstsc 的做法），
  也可以直接指定。**会话结束自动切回原分辨率**。支持情况：Windows 直接支持；Linux 需要 X11 + `xrandr`
  （Wayland 没有通用接口）；不支持的平台不会出现这个下拉。
...
**文件变更监控（FIM，随主机安全扫描一起跑）**

三大平台（Windows / Linux / macOS）都是**全盘**范围，只记录路径与元数据（大小 / 时间 / 权限），
不上传文件内容；只有内容审计白名单里的路径才计算哈希与文本差异。

- **覆盖每一个本地盘**：Windows 上按 Win32 接口枚举盘符并按驱动器类型过滤——**固定盘与可移动盘
  全部纳入**（不再只有 C 盘），网络驱动器 / 光驱不扫（那不是这台机器的状态，且会把扫描拖垮）。
  每个盘每一轮都分到自己的份额，互不相干的盘**并发**遍历（不同盘的 I/O 队列本来就是独立的）；
  同一棵树上的根仍按顺序走，以免同一片目录被重复扫。
  Linux/macOS 从 `/` 走下去即可覆盖全部本地文件系统（网络/伪文件系统自动排除）；macOS 额外跳过
  `/System/Volumes/Data`——它的内容已经通过 firmlink 出现在 `/Users`、`/Applications` 等位置，
  再走一遍等于整台机器扫两遍。
- **增量覆盖**：一次扫描有文件数与时间上限，而 Windows 的 `C:\` 动辄上百万文件。遍历按字典序进行，
  过去每一轮都在同一个位置被截断，靠后的目录（`C:\Users\...\Desktop` 就在其中）**永远轮不到**——
  这正是"在桌面新建的文件、目录识别不到"的原因。现在截断会记下断点，下一轮从那里接着走，
  走完一圈再从头开始；界面上会显示续扫点。
- **要害目录优先**：先扫用户数据与配置（Windows 的 `Users` / `ProgramData` / `Program Files` /
  `System32\drivers\etc` / `System32\Tasks`，Linux 的 `/etc` `/root` `/home` `/opt` `/usr/local` `/bin` 等，
  macOS 的 `/Users` `/Applications` `/Library/Launch*`），再扫整盘。第一轮就覆盖到人真正会动的地方。
- **目录也纳入**：新建 / 删除**目录**本身现在会被报出来（此前只记录普通文件，"新建了一个文件夹"
  在比对里根本不存在）。
- **不误报**：第一次走到某片区域时只默默建基线，不会把满盘文件报成"新增"；没走到的区域也不会
  被报成"删除"。读不进去的目录（权限不足）同理。
- 想缩小范围或调上限：模块参数 `fim_roots`（显式指定后不再自动加要害目录）、`fim_excludes`、
  `fim_max_files`、`fim_budget_sec`。

> 升级到本版后第一次扫描会**重建基线**（基线格式加入了目录），那一轮不报变更，属正常。
...
4. **策略控制**：设置 → Agent 自动升级 —— 维护窗口、按主机/分类豁免、连续失败熔断；`GET /api/v1/agents/auto-update-status` 可查看「为什么没升级」。
5. **任务状态**：`running → pending_verify → success/failed`；各平台均有独立升级助手日志，控制台可拉取现场证据。
...
- 与远程终端相同：Nginx 须关闭 `proxy_buffering` 与 `proxy_request_buffering`。
- 控制台查看自动升级状态或调用 `GET /api/v1/agents/auto-update-status` 看跳过原因。
